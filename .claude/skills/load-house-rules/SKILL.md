...
### And if the clone truly cannot happen: Drive has the whole rules

Built 2026-09-01, because "his PAT works" is one revoked token away from every
Cowork session silently running with no rules at all — the exact failure this
whole arrangement exists to stop. In the same Drive folder:

```
Gartera Handoffs / Cowork Instructions / house-rules-part-01.md  …  part-08.md
```

**That is the complete house-rules skill, verbatim, in eight parts.** Read every
one, in order. A part read alone is a lossy read presented as a complete one,
which is the single most expensive mistake in this estate.

Three things ride with it:

- **Use `download_file_content`, not `read_file_content`.** The latter
  markdown-escapes a `text/plain` file silently — `session_01…` comes back as
  `session\_01…` — and it still parses as valid JSON. Measured 2026-08-31.
- **The repo always wins.** Each part's header carries the mirror date. If you
  can reach `claude-audit` at all, read `SKILL.md` there and ignore the mirror.
- **Say which one you are running from**, in one line, to Garrett. An
  unannounced copy is how `PASTE-ME.md` drifted 73 lines and then 81, in two
  separate weeks.

The mirror is regenerated by `claude-audit/scripts/mirror_cowork_rules.py` and a
promise fails the build the moment `SKILL.md` moves past it, so a stale mirror is
loud rather than silent.

Also there: `00-READ-FIRST-cowork.md`, Cowork's equivalent of a repo's
`CLAUDE.md`. Short, and a **pointer**, not a copy. Read it in addition to the
