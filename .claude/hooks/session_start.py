...
def not_found_message(cwd):
    return "\n".join([
        ...
        "",
        "claude-audit is not reachable from this session (checked cwd and its",
        f"immediate children under {cwd}).",
        "",
        ...
        "",
        "If this is unexpected — claude-audit genuinely IS attached — say so",
...
def brief():
    ...
    msg = [
        "=== load-house-rules plugin · SessionStart ===",
        "",
        f"Found claude-audit at {audit_root}, but its own session_start.py",
